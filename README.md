...
이 가이드는 실제 브라우저의 TLS 핑거프린트를 모방하여 Python에서 Web스크레이핑 스크립트를 강화하기 위해 curl\_cffi를 사용하는 방법을 설명합니다.
...
- [Web Scraping을 위해 `curl_cffi`를 사용하는 방법](#how-to-use-curl_cffi-for-web-scraping)
  ...
  - [Step #4: 데이터 스크레이핑 로직 추가](#step-4-add-the-data-scraping-logic)
...
- `requests` API를 모방함
- 비동기 HTTP 요청 수행을 위한 `asyncio` 지원
- 각 요청마다 프록시 로테이션 지원
- HTTP/2.0 및 `WebSocket` 지원

## How It Works

HTTPS 요청를 전송하면 [TLS 핸드셰이크](https://www.cloudflare.com/learning/ssl/what-happens-in-a-tls-handshake/)가 수행되며, 고유한 TLS 핑거프린트를 생성합니다. HTTP 클라이언트는 웹 브라우저와 다르게 동작하기 때문에, 해당 핑거프린트로 자동화를 식별할 수 있으며 이는 앤チボット 방어를 활성화할 수 있습니다.
...
- **기본값이 아닌 cURL 플래그**: 정확성을 위해 `--ciphers`, `--curves`, 커스텀 헤더를 설정합니다.

이를 통해 요청가 실제 브라우저에서 온 것처럼 보이게 되어, 봇 탐지를 우회하는 데 도움이 됩니다.

## How to Use `curl_cffi` for Web Scraping

Walmart의 “Keyboard” 페이지를 스크레이핑해 보겠습니다:  
...
머신에 Python 3+가 설치되어 있는지 확인합니다. 그런 다음 `curl_cffi` 스크레이핑 프로젝트용 디렉터리를 생성합니다:
...
이 객체는 Requests와 유사한 고수준 API를 제공합니다. 이를 사용해 대상 페이지에 GET HTTP 요청를 수행할 수 있습니다:
...
Web스크레이핑을 수행하려면 [BeautifulSoup](https://www.crummy.com/software/BeautifulSoup/bs4/doc/) 같은 HTML 파싱 라이브러리도 필요합니다:
...
다음 예시는 페이지 제목만 스크레이핑하는 방법을 보여줍니다. `find()` 메서드를 사용해 [CSS selector](https://developer.mozilla.org/en-US/docs/Web/CSS/CSS_selectors)로 선택한 다음 `text` 속성으로 텍스트에 접근할 수 있습니다:
...
다음은 최종 `curl_cffi` Web스크레이핑 스크립트입니다:
...
3.  브라우저 버전은 해당 핑거프린트가 변경될 때만 추가됩니다. 어떤 버전이 건너뛰어졌다면, 이전 버전의 헤더를 사용해 그 버전을 가장할 수 있습니다.
...
`curl-cfii`는 [`Session`](https://requests.readthedocs.io/en/latest/user/advanced/#session-objects) 객체를 사용하여 Cookie, 헤더 또는 기타 세션별 데이터와 같은 특정 파라미터를 여러 요청에 걸쳐 유지할 수 있습니다.
...
이 결과는 세션이 서버에서 정의한 Cookie를 저장하는 등, 요청 전반에 걸쳐 상태를 유지하고 있음을 증명합니다.

### Proxy Integration

`requests` 라이브러리와 마찬가지로, `curl_cffi`는 `proxies` 객체를 통해 프록시 통합을 지원합니다:
...
`curl_cffi`는 [`AsyncSession`](https://curl-cffi.readthedocs.io/en/latest/api.html#curl_cffi.requests.AsyncSession) 객체를 통해 `asyncio` 기반 비동기 요청 수행을 지원합니다:
...
이는 `WebSocket`을 사용해 데이터를 동적으로 채우는 사이트나 API에서 실시간 데이터를 스크레이핑할 때 특히 유용합니다.

렌더링된 페이지를 스크레이핑하는 대신, 효율적인 데이터 검색을 위해 `WebSocket` 채널을 직접 타겟팅할 수 있습니다.
...
다음 표는 Web스크레이핑을 위한 `curl_cffi`와 기타 인기 있는 Python HTTP 클라이언트를 비교합니다:
...
`curl_cffi`는 Web스크레이핑에 수동 접근이 필요하며, 대부분의 코드를 직접 작성해야 합니다. 단순한 정적 웹사이트에는 효과적이지만, 동적이거나 보안 수준이 매우 높은 사이트를 다룰 때는 어려울 수 있습니다.

Bright Data는 다음과 같은 `curl_cffi` 대안을 제공합니다:

- [Scraping Browser API](https://brightdata.co.kr/products/scraping-browser): Puppeteer, Selenium, Playwright와 원활하게 통합되는 완전 관리형 클라우드 브라우저 인스턴스입니다. 이 브라우저에는 CAPTCHA 해결과 자동 프록시 로테이션이 내장되어 있습니다.
- [Web Scraper APIs](https://brightdata.co.kr/products/web-scraper): 사전 구성된 엔드포인트가 100개 이상의 인기 도메인에서 최신의 구조화된 데이터를 제공합니다.
- [No-Code Scraper](https://brightdata.co.kr/products/web-scraper/no-code): 코딩이 필요 없는 사용자 친화적 온디맨드 데이터 수집 서비스입니다.
- [Datasets](https://brightdata.co.kr/products/datasets): 다양한 웹사이트의 사전 구축 데이터셋을 탐색하거나, 특정 요구에 맞게 데이터 수집을 맞춤 구성할 수 있습니다.

지금 무료 Bright Data 계정을 생성하여 당사의 프록시 및 스크레이핑 솔루션을 테스트해 보십시오!